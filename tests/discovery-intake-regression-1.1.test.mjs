...
test('worksheet-first order is consistent between browser and server', () => {
  ...
  assert.deepEqual(contract.orderFor('home'), ['shoppingReason','improvementPriorities','ownershipDuration','upgradeSummary','claimExperience','permissionToAdvise']);
  assert.deepEqual(contract.orderFor('auto'), ['shoppingReason','improvementPriorities','annualMileage','vehicleCount','liabilityKnowledge','permissionToAdvise']);
});
...
test('prefilled answers advance to the first genuinely new question', () => {
  ...
  assert.equal(seed.questionOrder.length, 6);
...
test('HOME AUTO LIFE BUSINESS and BUYER keyword starts remain available', () => {
  const expected = { HOME:'home_review_address_requested', AUTO:'auto_need_requested', LIFE:'life_goal_requested', BUSINESS:'business_type_requested', BUYER:'buyer_closing_date_requested' };
...
test('auto SMS answer becomes a secure CoverageFit appointment handoff without inferred consent', () => {
  ...
  assert.equal(mapping.destination, '/pvx/appointment/');
  assert.equal(mapping.journey.experience, 'appointment_only');
...
test('secure bootstrap is POST-only, idempotent, and never redirects with PII', async () => {
  ...
  const makeRequest = (method, fields = body) => new Request('https://coveragefit.com/api/pvx/web-bootstrap', { method, headers:{ Origin:'https://408farmers.com', 'Content-Type':'application/x-www-form-urlencoded' }, body:method === 'POST' ? fields.toString() : undefined });
  ...
  assert.equal(first.headers.get('location'), '/pvx/web/?handoff=fresh');
  assert.equal(first.headers.get('x-coveragefit-bootstrap'), 'created');
  assert.equal(second.headers.get('location'), '/pvx/web/?handoff=resume');
  assert.equal(second.headers.get('x-coveragefit-bootstrap'), 'reused');
  const newSubmission = new URLSearchParams(body);
  newSubmission.set('bootstrap_id', 'pvxb_qrstuvwxyzabcdef');
  const third = await handlePvxWebBootstrap(makeRequest('POST', newSubmission), { store });
  assert.equal(third.headers.get('location'), '/pvx/web/?handoff=fresh');
  assert.equal(third.headers.get('x-coveragefit-bootstrap'), 'created');
