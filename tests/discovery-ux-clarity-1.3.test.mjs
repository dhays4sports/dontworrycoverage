...
test('secure opening now includes the focused technology first look', () => {
  const html = read('pvx/web/index.html');
  const css = read('assets/css/pvx-clarity-1.3.css');
  const firstLookCss = read('assets/css/pvx-tech-first-look.css');
  ...
  assert.match(html, /Continue my review/);
  assert.match(html, /Ask Dylan instead/);
  assert.match(css, /width: min\(620px, 100%\)/);
  assert.match(css, /place-items: center/);
  assert.match(css, /@media \(max-width: 430px\)/);
  assert.match(css, /forced-colors: active/);
  assert.match(css, /prefers-reduced-motion: reduce/);
  assert.match(firstLookCss, /pvx-tech-first-look__facts/);
  assert.match(firstLookCss, /@media \(max-width: 520px\)/);
});
...
test('continuity remains truthful for direct and carried entries', () => {
  ...
  assert.match(bootstrap, /Connected\. Opening your review/);
  assert.match(bootstrap, /handoffState !== 'fresh'/);
