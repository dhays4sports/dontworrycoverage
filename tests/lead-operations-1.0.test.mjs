...
test('producer projection keeps early permission separate from later contact request', () => {
  const normalized = normalizeLeadPayload(lead({
    landing_page:'https://408farmers.com/tech/',
    professional_program:'technology',
    professional_role:'software_engineering',
    professional_role_label:'Software engineering'
  })).value;
  ...
  assert.equal(projected.earlyLead.firstName,'Maya');
  assert.equal(projected.earlyLead.mobile,'4085551234');
  assert.equal(projected.earlyLead.professionalRoleLabel,'Software engineering');
  assert.equal(projected.earlyLead.housing,'homeowner');
  assert.equal(projected.earlyLead.callPermitted,true);
  assert.equal(projected.earlyLead.personalTextPermitted,true);
});
