...
export async function handlePVXCheckpoint(request, { store, journeyStore, operationsStore, now = new Date(), cryptoApi = globalThis.crypto, env = {}, waitUntil = null } = {}) {
  ...
  if (action === 'continue' || action === 'contact_only') {
    ...
    if(webJourney)await advancePvxWebJourney(webJourney,{store:journeyStore,now,env,waitUntil,stage:'snapshot_viewed',currentStep:contactRequested?'contact_requested':'continuing',details});
  ...
  if (action === 'contact') {
    ...
    if(webJourney)await advancePvxWebJourney(webJourney,{store:journeyStore,now,env,waitUntil,stage:'snapshot_saved',currentStep:'contact_requested',details});
  ...
  if(webJourney)await advancePvxWebJourney(webJourney,{store:journeyStore,now,env,waitUntil,stage:'snapshot_saved',currentStep:'saved',completedStage:'snapshot_saved',details:{reviewTopics:snapshot.whatDylanWouldLookAtFirst,topicResponses:record.topicResponses,preferredContactChannel:contact.preferredMethod,requestedContactTime:contact.bestTime,contactPurpose:contact.purpose,contactQuestion:contact.question,requestedProducerAction:consent.contact?'contact_requested':'none'}});
