...
test('TECH intake collects bounded role housing and review choice without eligibility inference', () => {
  ...
  assert.equal(housing.state,'tech_review_requested');
  assert.equal(housing.answers.housing,'renter');
  const review = routeSmsInbound({state:housing.state,intent:'tech',answers:{...role.answers,...housing.answers}},'3',{mode:'live'});
  assert.equal(review.state,'coveragefit_ready');
  assert.equal(review.answers.reviewTrack,'bundle');
  assert.match(review.reply,/does not confirm a discount, eligibility, or underwriting approval/i);
});
